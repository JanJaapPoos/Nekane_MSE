...
Mortality in the model resulted only from catches in the fishery. Mortality from natural causes such as predation, diseases, and scenesens was assumed to be negligible in comparison. Althought such absence of natural mortality impossible in reality, one could argue that the model thus mimiced long-lived species. The decrease in population numbers was thus the result of the catches ($C_i (a, y, s, p)$).

The mortality caused by the catches of the fishery reduce the population numbers. Because the model was seasonally structured, the population numbers for seasons 2 to $S$ were dependent on the previous season,

\begin{equation}
N_i (a, y, s+1, p) = N_i (a, y, s, p) - C_i (a, y, s, p). 
\end{equation}

the population numbers in season 1 depended on the numbers in season $S$ of the previous year,

\begin{equation}
N_i (a+1, y, 1, p) = N_i (a, y, S, p) - C_i (a, y, S, p). 
\end{equation}

Migration for each species was defined by an arrayx $M_i (a, s, em, im)$ that defined  immigration and emigration on a given stock relative to the stock sizes. The size of that array was defined by the number of age classes, seasons and number of areas. emigrants leave area $em$ and move to area $im$. The emigrated part of the population is then substracted from each of the areas, so that that population numbers per year and season remain unaffected by migration.   

Indivdual body growth was modelled by a von Bertalanffy growth equation to convert numbers to lengths, and an allometric equation to convert length to weight. The stock weights are thus calculated from the population numbers as

\begin{equation}
W_i(a,y,s,p) =  \alpha * (N_i (a, y, S, p) * L_{\infty_i} * (1-\exp^{(-K * a)}))^{\beta},
\end{equation}
 and the same is done for the catch weights.

The realized catches are the sum of all individual catches resulting from the Dynamic State Variable Model. The Dynamic State Variable model inputs consist of the expected individual catch rates, which are a random variable. This, random variable is normally distributed, and its mean $\hat c_i (a, y, s, p)$  is a function of population size, age dependent catchability $q_i (a)$, and fishing effort $E (y,s,p)$ in any year, season, and area.  

\begin{equation}
\hat c_i (a, y, s, p) = N_i (a, y, s, p) * q_i (a) * E(y,s,p).
\end{equation}
...
\textbf{Market value (Euro kg\textsuperscript{-1})}& \multicolumn{4}{c}{age} \\ 
                         & 1     & 2     & 3    & 4   \\ \hline
                      \\                      
species 1 and 2            &  778  & 1354   & 1780  & 2096  \\
\\
\hline 
\end{tabular}
}
...
\subsection{Case study parameterization}
 ...
 (six seasons) and spatially (two areas) explicit
The maximum age in the age-structured fish model was 4 years.
The two stocks are named "stock 1" and "stock 2". and the areas are named "area a" and "area b".
In the example used for our simulations, it was assumed that there were 100 new-borns per year for both stocks. The difference between the stocks is that all new-borns of stock 1 were born in "area a", while all new-borns of stock 2 were born in "area b".   

In our example, the age dependent catchability $q_i (a)$ was assumed independent of age $a$.

 A 20 \% constant migration during any season was set between areas. The migration inpopulation numbers ($M_i (a, s, im, em)$) was thus the proportion ($P (a, y, s, i, em, im)$) of the population available or vulnerable to fishing after migration occurs:

predict fish weight (W) at length (L). while assuming  ($L\infty= 20$, $k= 0.093$), with the age-weight relationship ($W= L\infty*(1-e^{-k*a})$); weight per fish was higher for the older fish.


 was set to $3 x 10^3$ Euro day\textsuperscript{-1}. These high fines combined with an assumed 100\% detection of exceeding quotas resulted in model results in which fishers comply with quota regulations.
