...
In order to allow the fishery to make spatial and temporal choices, the model was seasonally (six seasons) and spatially (two areas) explicit. The dynamics of two fish stocks were modelled by an age-structured model that is also seasonally and spatially explicit. The maximum age in the age-structured fish model was 4 years. The number of fish of stock $i$ of age $a$ at year $y$, in season $s$, and area $p$ was written as $N_i (a, y, s, p)$. The two stocks are named "stock 1" and "stock 2". and the areas are named "area a" and "area b". Individuals were born at age 0, at the start of each year $y$, in season 1. The number of new-born individuals in the model was independent of the size of the adult population, but was function of area $p$, and written as $R_i (p)$. Hence,
...
Moreover, combining the weight-based von Bertalanffy growth equation ($L\infty= 20$, $k= 0.093$), with the age-weight relationship ($W= L\infty*(1-e^{-k*a})$); weight per fish was higher for the older fish.
