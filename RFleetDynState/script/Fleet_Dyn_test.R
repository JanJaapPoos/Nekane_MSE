...
catchMean(hke)  <- array(c(rep(10,2), rep(20,2),rep(10,2), rep(5,2)), dim=c(8,4,2), dimnames=list(cat=1:8,season=as.character(1:4),option=c("a","b")))
catchSigma(hke) <- array(2                    ,dim=c(8,4,2), dimnames=dimnames(catchMean(hke)))

catchMean(mac)  <- array(0                      ,dim=c(8,4,2), dimnames=dimnames(catchMean(hke)))
catchSigma(mac) <- array(0.1                    ,dim=c(8,4,2), dimnames=dimnames(catchMean(hke)))
...
hkePrice <- array(c(8,10,12,16,20,22,25,30),dim=c(8,4), dimnames=list(cat=1:8, seasons=as.character(1:4)))
macPrice <- array(c(0),dim=c(8,4), dimnames=dimnames(hkePrice))
...
control <- DynState.control(spp1LndQuota= 125,  spp2LndQuota= 5,#tn
