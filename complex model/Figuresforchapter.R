...
screen( ind[1])
image(matrix(round(aperm(pop1[,(stab.model-1),,],c(2,1,3))), ncol=2), col=gray((0:64)/64), axes=F, main= "Species 1", ylab="Area" )
box()
axis(2,at= seq(0.,1,1),labels= c("S","N"), las=1)
axis(1,at= seq(0.,0.833,0.166),labels= seq(1:max(ages)))

screen( ind[2])
image(matrix(round(aperm(pop2[,(stab.model-1),,],c(2,1,3))), ncol=2), col=gray((0:64)/64), axes=F, main="Species 2", xlab="Age (years)", ylab="Area")
box()
axis(2,at= seq(0.,1,1),labels= c("S","N"), las=1)
axis(1,at= seq(0.,0.833,0.166),labels= seq(1:max(ages)))
...
legend("bottomright", inset=.05, legend=c("Catches","TAC"), pch=c(1,1,1), lwd=c(2.5,2.5),col=c("black","red"), bty='n',text.font =2, cex=1)
...
mtext(text="Yield per recruitment",side=2,line=0,font=2,outer=TRUE)
dev.off()
...
#Effort
##############################################################################
postscript(file="Effort.eps", onefile=FALSE, horizontal=FALSE,width=8.5,height=6.5)
...
par(oma=c(3,5,3,3),mar=c(1,1,1,1))

barplot(trip_percentage, col= mypalette, border=NA, xlim = c(1,NUMRUNS+1), xlab = " ", ylab = "Effort pattern (%)", xaxt = "n",space = 0)
...
barplot(as.matrix(daysmen[1:2,]), col=c("#808080","#CCCCCC"),xlim = c(0,max(season)), ylim=c(0, 8000),xlab = "n", ylab = "Mean effort(days)", xaxt = "n",space = 0)
axis( 1, at =seq(0.5,max(season)-0.5,1), labels = c(month.abb))
mtext("(b)", side=1, line = -1.3, adj = 0.01, font=2, cex = 1)

barplot(as.matrix(daysmen[3:4,]), col=c("#808080","#CCCCCC"), xlim = c(0,max(season)), ylim=c(0, 8000),xlab = " ", ylab = "Mean effort(days)", xaxt = "n",space = 0)
axis( 1, at =seq(0.5,max(season)-0.5,1), labels = c(month.abb))
mtext("(c)", side=1, line = -1.3, adj = 0.01, font=2, cex = 1)
...
barplot(as.matrix(daysmen[1:2,]), col=c("#808080","#CCCCCC"),xlim = c(0,max(season)), ylim=c(0, 8000),xlab = " ", ylab = "Mean effort(days)", xaxt = "n",space = 0)
axis( 1, at =seq(0.5,max(season)-0.5,1), labels = c(month.abb))
mtext("(e)", side=1, line = -1.3, adj = 0.01, font=2, cex = 1)

barplot(as.matrix(daysmen[3:4,]), col=c("#808080","#CCCCCC"), xlim = c(0,max(season)), ylim=c(0, 8000),xlab = " ", ylab = "Mean effort (days)", xaxt = "n",space = 0)
axis( 1, at =seq(0.5,max(season)-0.5,1), labels = c(month.abb))
mtext("(f)", side=1, line = -1.3, adj = 0.01, font=2, cex = 1)
...
#ECONOMICS
##############################################################################
postscript(file="Economics.eps", onefile=FALSE, horizontal=FALSE,width=8.5,height=6.5)
load("~/Dropbox/BoB/MSE/Git/Nekane_MSE/complex model/1quota12seasons.RData")
par(par(no.readonly=TRUE))
...
mtext("(e)", side=3, line = -1.3, adj = 0.01, font=2, cex = 1)
...
axis(1, at = c(MPstart), labels = c("MP"))
abline(v=MPstart, lty=2)
legend("bottomright", inset=.05, legend=c("Net revenue","Gross revenue", "Fuel cost", "Annual fine"), pch=c(1,1,1,1), lwd=c(2,2,2,2),col=c("black","gray","blue","red"), bty='n',text.font =2, cex=1)
mtext("(f)", side=3, line = -1.3, adj = 0.01, font=2, cex = 1)
